...
1. Resolve the repository that owns the requested implementation and use its workspace-root `openspec/`. Use the workbench source repository's own `openspec/` only when changing the workbench itself.
