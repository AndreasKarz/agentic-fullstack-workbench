...
End-to-end workflow for authoring and maintaining Playwright tests using `frontend-playwright-cli`. The three sections below can be used independently:

- **Planning** — explore the app, produce a spec file describing what to test.
- **Generate** — turn a spec into Playwright test files. Update the spec if it's vague or stale.
- **Heal** — diagnose failing tests, fix the code, reconcile the spec with reality.

All three lean on the same mechanic: run `npx playwright test --debug=cli` in the background, then `playwright-cli attach tw-XXXX` to drive the paused page interactively. See [playwright-tests.md](playwright-tests.md) for the debug/attach mechanics and [test-generation.md](test-generation.md) for how every `frontend-playwright-cli` action emits Playwright TypeScript.
...
Walk the scenario's `Steps:` one by one with `frontend-playwright-cli`, treating the spec as the plan and the live app as the source of truth. If a step is vague ("click the button" — which button?), references an element that no longer exists, or contradicts the app's actual behaviour, use your judgement: update the spec to match what the app really does, then keep going. Editing the spec mid-generation is expected.
...
Rehearse the corrected interaction with `frontend-playwright-cli` — the generated code in the output is what you paste back into the test.
...
| How `frontend-playwright-cli` actions become TS | [test-generation.md](test-generation.md) |
