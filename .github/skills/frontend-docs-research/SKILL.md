...
3. Use the `afw-microsoft-docs` MCP server first:
...
5. If `afw-microsoft-docs` tools are unavailable, state that limitation and continue from local repo evidence plus other official docs only when the task can still be completed safely

## Query Strategy

Use targeted queries that include the concrete task, version, and platform where possible.

| Task Area | `afw-microsoft-docs` query examples |
|---|---|
...
|---|---|---|
| Coding from memory on a new task | Practices drift quickly across React, TypeScript, auth, and tooling | Query `afw-microsoft-docs` first |
...
- Use `afw-microsoft-docs` first for every new frontend task
