...
5. If Microsoft Docs MCP tools are unavailable, state that limitation and continue from local repo evidence plus other official docs only when the task can still be completed safely

## Query Strategy

Use targeted queries that include the concrete task, version, and platform where possible.

| Task Area | Microsoft Docs MCP query examples |
|---|---|
...
|---|---|---|
| Coding from memory on a new task | Practices drift quickly across React, TypeScript, auth, and tooling | Query Microsoft Docs MCP first |
...
- Use Microsoft Docs MCP Server first for every new frontend task
